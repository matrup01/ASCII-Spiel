...
class Event:
    ...
    def select(self,status): #gibt ein Event aus (je nach tyletype unterschiedlich gewichtet) 0 --> Kampf, 1 --> Händler, 2--> info, 3 --> Schatz, 4--> Boss, 5--> Schmied
        self.status = status
        if self.status[4] == 210:
            select = 4
        else:
            bossweight = int(round(self.status[4] / 100,0))
            if status[2] == 0:
                weight = [1,1,1,1,bossweight,1]
            elif status[2] == 1:
                weight = [0,1,0,1,bossweight,0]
            else:
                weight = [1,1,1,1,bossweight,1]
